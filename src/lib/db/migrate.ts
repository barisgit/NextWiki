...
const getDb = () => {
  ...
  if (!connectionString) {
    throw new Error('DATABASE_URL environment variable is still not set after loading .env!');
  }
  
  console.log('Connection string found in process.env');
  
  // Create database client based on connection string
  if (connectionString.includes('pooler.internal.neon') || connectionString.includes('.neon.tech')) {
...
async function runMigrations(force = false) {
  try {
    ...
    const connectionString = process.env.DATABASE_URL || '';
    console.log("Migration script using connection string (sanitized):", 
      connectionString.replace(/postgresql:\/\/([^:]+):([^@]+)@/, 'postgresql://$1:****@'));
    ...
    try {
      if (connectionString.includes('pooler.internal.neon') || connectionString.includes('.neon.tech')) {
        ...
        migrationsResult = await migrateNeon(db as NeonHttpDatabase<typeof schema>, { 
          migrationsFolder: 'drizzle',
        });
        console.log('Migrations completed successfully using Neon driver');
      } else {
        console.log("Using PostgreSQL migrator..."); // Added log
        migrationsResult = await migratePg(db as NodePgDatabase<typeof schema>, { 
          migrationsFolder: 'drizzle',
        });
        console.log('Migrations completed successfully using PostgreSQL driver');
      }
      ...
      if (migrationError.message && migrationError.message.includes('already exists')) {
        console.warn('Some relations already exist. This likely means migrations were already applied.');
        console.warn('Original error:', migrationError.message);
        
        if (force) {
          console.log('Force flag is set, attempting to continue with other migrations...');
          // Here you could implement logic to run only specific migrations
          // but for now we'll just acknowledge the issue
        } else {
          console.log('Use the --force flag to attempt running migrations anyway.');
          console.log('Or manually check your database schema to ensure it matches your migrations.');
        }
      } else {
